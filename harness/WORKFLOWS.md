...
# Workflows — @awsysco/sdk

> Operational patterns enforced by the harness for agent `awsysco-ts-sdk`.
...
This agent (`awsysco-ts-sdk`, trust: `worker`) can delegate to:
...
### Namespace: `awsysco-ts-sdk-worker`
...
| `429 Too Many Requests` | Budget exceeded or rate limited | Check `harness-ctl budget awsysco-ts-sdk` |
...
| `"cross-namespace read"` | memory: no grant | Only query own namespace (`awsysco-ts-sdk-worker`) |
...
- [ ] Budget checked if doing many tool calls: `harness-ctl budget awsysco-ts-sdk`
