...
> Tools available to agent `awsysco-ts-sdk`. Every call goes through the harness
...
Namespace: `awsysco-ts-sdk-worker` — other agents cannot read this unless explicitly granted.
...
All paths are relative to the project root (`/Users/patrickbertsch/dev/awsysco-ts-sdk`).
...
**Example — read:**
```
file_ops(op="read", path="src/index.ts")
```
...
Attempting to call `shell` will return: `"shell skill not permitted for agent awsysco-ts-sdk"`
