...
identity is `harness-ctl mcp-bridge awsysco-ts-sdk`, registered as a stdio MCP server:

```json
{
  "mcpServers": {
    "harness-awsysco-ts-sdk": {
      "type": "stdio",
      "command": "harness-ctl",
      "args": ["mcp-bridge", "awsysco-ts-sdk"]
    }
  }
}
```
